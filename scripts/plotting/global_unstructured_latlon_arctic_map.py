"""
Generate arctic maps of 2-D fields
completely redundant with scripts/plotting/global_unstructured_latlon_map.py
here explicit calls projection = 'arctic'

Functions
---------
arctic_latlon_map(adfobj)
...
def global_unstructured_latlon_arctic_map(adfobj):
    ...
    for var in var_list:
        ...
        if var in res:
            ...
            adfobj.debug_log(f"arctic_latlon_map: Found variable defaults for {var}")
        ...
        if odata is None:
            dmsg = f"No regridded test file for {base_name} for variable `{var}`, arctic lat/lon mean plotting skipped."
            adfobj.debug_log(dmsg)
            continue
        ...
        if (not o_has_dims['has_lat']) or (not o_has_dims['has_lon']):
            print(f"\t = Unstructured grid, so arctic map for {var} does not have lat and lon")
        ...
        for case_idx, case_name in enumerate(adfobj.data.case_names):
            ...
            if mdata is None:
                dmsg = f"No climo file for {case_name} for variable `{var}`, arctic lat/lon mean plotting skipped."
                adfobj.debug_log(dmsg)
                continue
            ...
            if (not has_dims['has_lat']) or (not has_dims['has_lon']):
                print(f"\t = Unstructured grid, so arctic map for {var} for case {case_name} does not have lat and lon")
            ...
            if not has_dims['has_lev']:  # strictly 2-d data          
                ...
                for s in seasons:
                    ...
                    pf.plot_unstructured_map_and_save(plot_name, case_nickname, adfobj.data.ref_nickname,
                                                      ...
                                                      dseasons[s], pseasons[s], wts,
                                                      obs=adfobj.compare_obs, 
                                                      projection='arctic', **vres)
...
def aod_latlon(adfobj):
    ...
    for case in test_case_names:
        ...
        #Skip this variable/case if the climo file doesn't exist:
        if ds_case is None:
            dmsg = f"No test climo file for {case} for variable `{var}`, arctic lat/lon plots skipped."
            adfobj.debug_log(dmsg)
            continue
        else:
    ...
    if not adfobj.compare_obs:
        ...
        if ds_base is None:
            dmsg = f"No baseline climo file for {base_name} for variable `{var}`, arctic lat/lon plots skipped."
            adfobj.debug_log(dmsg)
        else:
            # Round lat/lons so they match obs
            # NOTE: this is neccessary due to small fluctuations in insignificant decimal places
