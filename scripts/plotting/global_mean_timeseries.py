...
def global_mean_timeseries(adfobj):
    ...
    for field in adfobj.diag_var_list:
        ...
        if len(ts_files) != 1:
            ...
            errmsg += f" Multiple files were found for the variable '{field}', so it will be skipped."
            warnings.warn(errmsg)
            continue
        #End if
        ...
        if field in emislist:
            ref_ts_da = surface_emission(ref_ts_da)
        elif field in cblist:
        ...
        # reference time series global average
            ...
            if not has_lat_ref:
                warnings.warn(
                    f"\t Variable named {field} is missing a lat dimension, cannot continue to plot."
                )                
                continue
            # End if
        ...
        for case_name in adfobj.data.case_names:
            c_ts_files = adfobj.data.get_timeseries_file(case_name, field)
            # If no files exist, try to move to next variable. --> Means we can not proceed with this variable, and it'll be problematic later.
            if not c_ts_files:
                errmsg = f"Time series files for case: {case_name} and variable '{field}' not found.  Script will continue to next variable."
                warnings.warn(errmsg)
                continue
            #End if
