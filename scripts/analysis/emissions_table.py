...
def emissions_table(adfobj, Climate=None,**kwargs):
    '''
    Calculate emissions table for all the availabe species

        CO_EMIS (Tg/yr), CO_EMIS_elevated (Tg/yr), CO_EMIS_MEGAN (Tg/yr)
    ...
    MODIFICATION HISTORY:
        Behrooz Roozitalab, 17, APR, 2026: VERSION 1.00
        - Initial version, based on aerosol_gas_tables.py

    '''

    #Notify user that script has started:
    msg = "\n   Calculating emissions table..."
    print(f"{msg}\n  {'-' * (len(msg)-3)}")
    ...
    for i,case in enumerate(nicknames_list):
        ...
        for v in ListVars:
            if v[:2]=='SF':
                if v not in ['SF6','SFnum_a1','SFnum_a2','SFnum_a3','SFnum_a4']:
                    VARIABLES.append(v[2:])
        ...
        dic_SE = fill_dic_SE_Emis(adfobj, dic_SE, VARIABLES, ListVars, ext1_SE, AEROSOLS, MW, AVO, gr, Mwair)
    ...
    #-------------------------
    if len(VARIABLES) > 0:
        print("\tMaking table for all emissions")
        make_table_Emis(vars=VARIABLES, chem_type='all', **table_kwargs)
...
def fill_dic_SE_Emis(adfobj, dic_SE, variables, ListVars, ext1_SE, AEROSOLS, MW, AVO, gr, Mwair):
    """
    ...
    for var in variables:
        ...
        for key in var_keys:
            ...
            # for MEGAN emissions
            # original unit: [kg/m2/s]
            if 'MEG_'+key in ListVars: 

                dic_SE[var+'_MEG']['MEG_'+key+ext1_SE]=1          
            else:
                dic_SE[var+'_MEG']['PS'+ext1_SE]=0.   
                
            # for CMXF:
            # original unit: [kg/m2/s]
            if key+'_CMXF' in ListVars: 
                #if var=='SO4':
                if var in ['SO4','so4_a1','so4_a2','so4_a3','so4_a5']:
                
                    dic_SE[var+'_CMXF'][key+'_CMXF'+ext1_SE]=32.066/115.11    
                else:
                    dic_SE[var+'_CMXF'][key+'_CMXF'+ext1_SE]=1
            else:
                dic_SE[var+'_CMXF']['PS'+ext1_SE]=0.   
...
def make_Dic_scn_var_comp_2D(adfobj, variables, current_dir, dic_SE, current_files, ext1_SE, AEROSOLS): 
    ...
    for current_var in variables:
        components=[current_var+'_SF',current_var+'_CMXF',current_var+'_MEG']
...
def calc_budget_data_Emis(current_var, Dic_scn_var_comp, area, inside, num_yrs, duration, AEROSOLS):
    """
    ...
    # Elevated Emissions
    try:
        spc_cmxf=Dic_scn_var_comp[current_var][current_var+'_CMXF']
    except:
        spc_cmxf=0
    tmp_cmxf=spc_cmxf
    cmxf=np.ma.masked_where(inside==False,tmp_cmxf*area)  #convert Kg/m2/s to Tg/yr
    CMXF = np.ma.sum(cmxf*duration*1e-9)/num_yrs
    chem_dict[f"{current_var}_EMIS_elevated (Tg{specifier}/yr)"] = np.round(CMXF,5)



   # Surface Emissions Megan
    try:
        spc_sf_meg=Dic_scn_var_comp[current_var][current_var+'_MEG']
    except:
        spc_sf_meg=0
    tmp_sf_meg=spc_sf_meg
    sf_meg=np.ma.masked_where(inside==False,tmp_sf_meg*area)  #convert Kg/m2/s to Tg/yr
    SF_MEG = np.ma.sum(sf_meg*duration*1e-9)/num_yrs
    chem_dict[f"{current_var}_EMIS_MEGAN (Tg{specifier}/yr)"] = np.round(SF_MEG,5)
...
def make_table_Emis(adfobj, vars, chem_type, Dic_scn_var_comp, areas, case_names, nicknames, durations, insides, num_yrs, AEROSOLS):
    """
    ...
    output_csv_file = output_location / f'ADF_amwg_{chem_type}_emissions_table.csv'
