...
def regrid_and_vert_interp_2(adf):
    """
    ...
    for case_idx, case_name in enumerate(case_names):
        ...
        for var in var_list:
            ...
            if var in adf.data.ref_var_nam:
                ...
                print(f"No reference data available for {var}.")
                continue
            if not ref_ds:
                print(f"Missing reference data for {var}. Skipping.")
                continue
            if not model_ds:
                print(f"Missing model data for {var}. Skipping.")
                continue
            ...
            if vert_type == 'hybrid':
                ...
                if ps_regridded_path.exists():
                    ...
                    print("\t    INFO: Regridding PS on the fly for hybrid interpolation.")
            ...
            print(f"\t    INFO: Saving regridded file: {regridded_file_loc}")
...
def _create_clean_grid(ds):
    """
    Creates a minimal, CF-compliant xarray Dataset for xesmf.
    """
    ...
    if 'lat_bnds' in ds and 'lon_bnds' in ds:
        lat_v = np.append(ds.lat_bnds.values[:, 0], ds.lat_bnds.values[-1, 1])
        lon_v = np.append(ds.lon_bnds.values[:, 0], ds.lon_bnds.values[-1, 1])
...
def _handle_vertical_interpolation(da, vert_type, source_ds, ps_da=None):
    ...
    if vert_type == "hybrid":
        ...
        return utils.lev_to_plev(da, ps_da, hyam, hybm, P0=p0, convert_to_mb=True, new_levels=DEFAULT_PLEVS)

    elif vert_type == "height":
        pmid = source_ds.get('PMID')
        if pmid is None:
            raise ValueError("'PMID' is required for height vertical interpolation.")
        return utils.pmid_to_plev(da, pmid, convert_to_mb=True, new_levels=DEFAULT_PLEVS)

    elif vert_type == "pressure":
        return utils.plev_to_plev(da, new_levels=DEFAULT_PLEVS, convert_to_mb=True)
