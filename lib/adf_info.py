...
class AdfInfo(AdfConfig):
    ...
    def __init__(self, config_file, debug=False):
        ...
        if self.__compare_obs:
            ...
            baseline_hist_str_raw = self.get_baseline_info("hist_str")
            ...
            if baseline_ts_done:
                ...
                found_yr_range = np.arange(
                    found_syear_baseline, found_eyear_baseline, 1
                )
                ...
                if syear_baseline not in found_yr_range:
                    ...
                    msg += (
                        f"{data_name}, using first found year: {found_syear_baseline}\n"
                    )
                    print(msg)
                    syear_baseline = found_syear_baseline
                ...
                if eyear_baseline not in found_yr_range:
                    msg = (
                        f"Given end year '{eyear_baseline}' is not in current dataset "
                    )
                    msg += (
                        f"{data_name}, using first found year: {found_eyear_baseline}\n"
                    )
                    print(msg)
                    eyear_baseline = found_eyear_baseline
            # End if
            ...
            if any(baseline_hist_locs):
                for hist_str in baseline_hist_str:
                    ...
                    base_climo_yrs = [
                        int(str(i).partition(f"{hist_str}.")[2][0:4]) for i in file_list
                    ]
    ...
    def hist_str_to_list(self, conf_var, conf_val):
        if isinstance(self.get_cam_info("hist_str", required=True), list):
            ...
            hist_str = [
                conf_val
            ]  # make a nested list [ncases,nfiles] of the given value
    ...
    def get_climo_yrs_from_ts(self, input_ts_loc, case_name):
        ...
        if not input_location.is_dir():
            errmsg = (
                f"Time series directory '{input_ts_loc}' not found.  Script is exiting."
            )
        ...
        if "time_bnds" in cam_ts_data:
            ...
            time = xr.DataArray(
                cam_ts_data["time_bnds"].load().mean(dim="nbnd").values,
                dims=time.dims,
                attrs=time.attrs,
            )
