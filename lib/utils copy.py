...
def grid_timeseries(adfobj, **kwargs):
    ...
    for var in diag_var_list:
        ...
        rgdata = rgdata.assign_coords(time=original_time)
        #print("regridded_file_loc",rgdata.time,"\n\n")
        if isinstance(rgdata, xr.DataArray):
            rgdata = rgdata.to_dataset(name=var)
...
def regrid_atm_se_data_bilinear(regridder, data_to_regrid, var_name, comp_grid="ncol"):
    ...
    out = regridder(da)
    out = out.squeeze("dummy", drop=True)
    out.name = var_name
    out = out.transpose("time", "lev", "lat", "lon")
